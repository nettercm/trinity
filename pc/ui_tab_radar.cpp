	...
	void f1::UpdateRadar(float a, int b)
	{
		...
		while(i<history_index)
		{
			if(radar_checkBox_show_ir_north->Checked)
			{
				...
				if( !(radar_checkBox_use_lines->Checked) ||  (abs(inputs_history[i].ir[1] - inputs_history[i-1].ir[1]) > 40) )
				{
			...
			if(radar_checkBox_show_ir_far_north->Checked)
			{
				...
				if( !(radar_checkBox_use_lines->Checked) ||  (abs(inputs_history[i].ir[3] - inputs_history[i-1].ir[3]) > 40) )
				{
